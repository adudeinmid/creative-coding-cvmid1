class App {
  ...
  setup() {
    ...
    for (let j = 0; j < 480; j += this.stepY) {
      for (let i = 0; i < 640; i += this.stepX) {
        this.grid.push(
          new Rectangle(this.offsetX + i * this.scale,this.offsetY + j * this.scale,40,this.ctx)
        );      }
    }
  ...
  draw() {
    ...
    this.grid.forEach((rectangle, index) => {
      const color = this.rgb[index];
      rectangle.color = `rgba(${color.r}, ${color.g}, ${color.b},0.4)`;
      rectangle.draw();
